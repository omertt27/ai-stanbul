...
function Chatbot({ onDarkModeToggle }) {
  ...
  const [messages, setMessages] = useState(() => {
    // Load message history from localStorage on init
    const savedMessages = localStorage.getItem('istanbul-chatbot-messages');
  ...
  const [darkMode, setDarkMode] = useState(() => {
    ...
    const savedDarkMode = localStorage.getItem('istanbul-chatbot-darkmode');
  ...
  const [apiHealth, setApiHealth] = useState({ healthy: true, message: '' });
  ...
  useEffect(() => {
    const unsubscribe = subscribeToNetworkStatus((online) => {
      ...
      if (online && currentError?.type === ErrorTypes.OFFLINE) {
        // Clear offline errors when back online
        setCurrentError(null);
        setConnectionAttempts(0);
        
        // Auto-retry last failed message if available
        if (lastFailedMessage && retryAction) {
          console.log('🔄 Auto-retrying after reconnection');
          setTimeout(() => {
            retryAction();
          }, 1000);
        }
    ...
    const checkHealth = async () => {
      try {
        ...
        if (!health.healthy && currentError?.type !== ErrorTypes.SERVER) {
          setCurrentError({
        ...
        setApiHealth({ healthy: false, message: error.message });
      }
    };
  ...
  }, [currentError, retryAction, lastFailedMessage]);

  // Clear error when user starts typing
  useEffect(() => {
    if (input.trim() && currentError) {
      setCurrentError(null);
    }
  }, [input, currentError]);
  ...
  const handleError = (error, context = '', originalMessage = null) => {
    ...
    setCurrentError({
      ...
      type: errorType,
      context,
      isRetryable: error.isRetryable || [ErrorTypes.NETWORK, ErrorTypes.TIMEOUT, ErrorTypes.SERVER].includes(errorType)
    });
    
    // Store failed message for retry
    if (originalMessage) {
      setLastFailedMessage(originalMessage);
    }
    
    // Increment connection attempts for backoff
    ...
    if (errorType !== ErrorTypes.OFFLINE) {
      const errorChatMessage = {
        role: 'assistant',
        content: `❌ ${userMessage}\n\n${error.isRetryable ? 'You can try again or rephrase your question.' : 'Please try a different approach.'}`,
        isError: true
      };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl w-full px-4">
              <div 
                ...
                className={`p-4 rounded-xl border transition-all duration-200 cursor-pointer hover:shadow-lg hover:scale-105 ${
                  darkMode 
                    ...
                    : 'bg-white border-gray-300 hover:bg-blue-50 hover:border-blue-400 shadow-sm hover:shadow-md'
                }`}
              >
              ...
              <div 
                ...
                className={`p-4 rounded-xl border transition-all duration-200 cursor-pointer hover:shadow-lg hover:scale-105 ${
                  darkMode 
                    ? 'bg-gray-800 border-gray-700 hover:bg-gray-750 hover:border-gray-600' 
                    : 'bg-white border-gray-300 hover:bg-red-50 hover:border-red-400 shadow-sm hover:shadow-md'
                }`}
              >
                <div className={`font-semibold mb-2 transition-colors duration-200 ${
                  darkMode ? 'text-white' : 'text-gray-900'
              ...
              <div 
                ...
                className={`p-4 rounded-xl border transition-all duration-200 cursor-pointer hover:shadow-lg hover:scale-105 ${
                  darkMode 
                    ? 'bg-gray-800 border-gray-700 hover:bg-gray-750 hover:border-gray-600' 
                    : 'bg-white border-gray-300 hover:bg-green-50 hover:border-green-400 shadow-sm hover:shadow-md'
                }`}
              >
                <div className={`font-semibold mb-2 transition-colors duration-200 ${
                  darkMode ? 'text-white' : 'text-gray-900'
              ...
              <div 
                ...
                className={`p-4 rounded-xl border transition-all duration-200 cursor-pointer hover:shadow-lg hover:scale-105 ${
                  darkMode 
                    ? 'bg-gray-800 border-gray-700 hover:bg-gray-750 hover:border-gray-600' 
                    : 'bg-white border-gray-300 hover:bg-purple-50 hover:border-purple-400 shadow-sm hover:shadow-md'
                }`}
              >
                <div className={`font-semibold mb-2 transition-colors duration-200 ${
                  darkMode ? 'text-white' : 'text-gray-900'
        ...
        <div className="w-full max-w-4xl mx-auto">
          <div className="flex space-x-4">
            <input
              ...
              placeholder="Ask me about Istanbul restaurants, attractions, neighborhoods, or anything else..."
              disabled={loading}
              className={`flex-1 p-3 border rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors duration-200 ${
                darkMode 
                        ...
                        {msg.type && (
                          <span className={`ml-2 px-1 py-0.5 rounded text-xs ${
                            msg.type === 'error' 
                              ...
                              : msg.type === 'restaurant-recommendation' 
                                ? 'bg-green-100 text-green-600'
                                : msg.type === 'places-recommendation'
