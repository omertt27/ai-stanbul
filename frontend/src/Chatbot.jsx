            ...
            {messages.map((msg, index) => (
              <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                {msg.role === 'user' ? (
                  ...
                  <div className="bg-gray-700 text-white rounded-2xl px-6 py-4 max-w-xs shadow-lg">
                    <p className="text-sm font-medium">{msg.content}</p>
                  </div>
                ) : (
                  // AI message - Mixed GPT/Gemini style with ChatGPT background
                  <div className="bg-gray-50 rounded-2xl p-6 max-w-2xl shadow-sm border border-gray-200">
