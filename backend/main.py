...
async def ai_istanbul_stream(request: Request):
    ...
    try:
        ...
        try:
            # Use the same logic as the regular endpoint to get the message
            # [Copy the main logic from the regular /ai endpoint]
            ...
            # Get the response message using the same logic as the regular endpoint
            message = ""
            ...
            if is_restaurant_query:
                try:
                    ...
                    if places_data.get('results'):
                        ...
                        message = restaurants_info
                    else:
                        message = "Sorry, I couldn't find any restaurants matching your request in Istanbul."
                        
                except Exception as e:
                    print(f"Error fetching Google Places data: {e}")
                    message = "Sorry, I encountered an error while searching for restaurants. Please try again."
                    ...
                    if not filtered_places:
                        ...
                        for district in sorted(districts):
                            ...
                            for place in places_in_district[:3]:  # Top 3 places per district
                                district_info += f"   - {place.name} ({place.category})\n"
                            district_info += "\n"
                        message = district_info
                else:  # general attraction query
                    filtered_places = [p for p in places if p.category and p.category.lower() in ['historical place', 'mosque', 'church', 'park']]
                
                if not message:  # Only process if we haven't set a message yet
                    # Apply location filter if location was extracted
                    if extracted_location and filtered_places:
                        location_lower = extracted_location.lower()
                        original_count = len(filtered_places)
                    ...
                    if filtered_places:
                        ...
                        if extracted_location:
                            message = f"Sorry, I couldn't find any {'museums' if is_museum_query else 'places'} in {extracted_location.title()} in my database. Try asking about a different district or general attractions in Istanbul."
                        else:
                            message = f"Sorry, I couldn't find any {'museums' if is_museum_query else 'attractions'} in my database."
...
- Some events may be in Turkish only - check beforehand"""
                ...
                # Use fallback responses for other queries
                places = db.query(Place).all()
