...
class TransportationIntentClassifier:
    ...
    def _build_patterns(self) -> Tuple[List[str], List[str]]:
        ...
        high_confidence = [
            ...
            # === RUSSIAN ===
            r'\bкак\s+(добраться|доехать|попасть|дойти|проехать)\b',  # как добраться/доехать
            r'\b(от|из)\s+.+\s+(до|в|на)\b',  # от X до Y, из X в Y
            r'\b(маршрут|путь|дорога)\s+(от|до|в|к|на)\b',  # маршрут до
            r'\b(какой|какое|какая)\s+(метро|трамвай|автобус|паром)\b',  # какой метро
            r'\b(идет|ходит|едет)\s+(ли|до)\b',  # идет ли, ходит до
            r'\b(аэропорт|в\s+аэропорт|из\s+аэропорта)\b',  # airport
            r'\b(сколько|как\s+долго)\s+(времени|ехать|добираться)\b',  # сколько времени
            r'\b(босфор|пересечь|через)\b',  # Bosphorus crossing
            r'\b(мне\s+нужно|хочу)\s+(добраться|доехать|попасть)\b',  # мне нужно добраться
            r'\b(отвези|довези)\s+(меня|нас)\b',  # отвези меня (take me)
            r'\b(паром|паромы)\s+(до|в|на|из)\b',  # паром до (ferry to)
            r'\b(как)\s+(взять|сесть|использовать)\s+(паром|метро|автобус)\b',  # как взять паром
        ]
        ...
        normal = [
            ...
            # === RUSSIAN ===
            r'\b(ехать|идти|добраться|доехать|приехать)\s+(до|в|на|к)\b',  # ехать до
            r'\b(отсюда|оттуда|туда|сюда)\b',  # from here/there
            r'\b(метро|трамвай|автобус|паром|маршрутка|поезд)\b',  # transit modes
            r'\b(станция|остановка|причал|терминал)\b',  # stations/stops
            r'\b(пересадка|пересаживаться)\b',  # transfer
            r'\b(лучший|быстрый|простой)\s+(путь|маршрут|способ)\b',  # best/fastest route
            r'\b(нужно|хочу|надо)\s+(добраться|доехать|попасть)\b',  # need to get
            r'\b(общественный\s+транспорт)\b',  # public transport
        ]
