...
class IstanbulTransportationRAG:
    ...
    def _get_same_line_neighbors(self, station_id: str) -> List[str]:
        ...
        # Phase 1: Origin → Nearest Ferry Terminal
        logger.info(f"🛳️ Finding route to nearest ferry terminal from {origin}")
        terminal_route = None
        
        for terminal_id in preferred_terminals:
            # Try to find a route to this terminal
            route_to_terminal = self._find_path(origin, terminal_id, max_transfers=2)
            if route_to_terminal:
                terminal_route = route_to_terminal
                logger.info(f"✅ Found route to ferry terminal: {terminal_id}")
                break
        
        if not terminal_route:
            logger.warning(f"⚠️ Could not find route to any ferry terminal from {origin}")
            return None  # Could not find route to any terminal
        
        # Phase 2: Ferry Terminal → Island
        logger.info(f"🛳️ Routing from ferry terminal to island: {destination}")
        island_route = self._find_path(terminal_route.destination, destination, max_transfers=0)
        
        if not island_route:
            logger.warning(f"⚠️ Could not find direct route from terminal to island: {destination}")
            return None
        
        # Combine routes: Origin → Terminal + Terminal → Island
        combined_steps = terminal_route.steps + island_route.steps
        combined_lines = list(set(terminal_route.lines_used + island_route.lines_used))
        total_time = terminal_route.total_time + island_route.total_time
        total_distance = terminal_route.total_distance + island_route.total_distance
        
        # Create combined route
        combined_route = TransitRoute(
            origin=origin,
            destination=destination,
            total_time=total_time,
            total_distance=total_distance,
            steps=combined_steps,
            transfers=terminal_route.transfers + island_route.transfers,
